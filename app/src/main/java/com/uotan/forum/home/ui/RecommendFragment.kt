...
import com.uotan.forum.databinding.FragmentRecommendBinding
import com.uotan.forum.home.ui.adapter.RecommendAdapter
...
class RecommendFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 为 adapter 赋予 RecommendAdapter 实例
        adapter = RecommendAdapter().apply {
            onItemClick = { selectedItem, itemLayout ->
                val intent = Intent(requireContext(), ThreadsActivity::class.java)
                    .putExtra("url", selectedItem.url).putExtra("title", selectedItem.title)
        ...
        // 首次加载
        viewModel.loadInitialRecommendData(
            isRefresh = false,
            onSuccess = {},
            onException = {
        ...
        binding.srlRoot.setOnLoadMoreListener {
            viewModel.loadMoreRecommendData(
                onSuccess = {
    ...
    private fun obverse() {
        ...
        viewModel.isRecommendLastPage.observe(viewLifecycleOwner) {
