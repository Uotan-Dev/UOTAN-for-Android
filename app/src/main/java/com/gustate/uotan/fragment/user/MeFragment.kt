...
class MeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(view.rootView) { _, insets ->
            ...
            binding.toolbar
                .updateLayoutParams<ViewGroup.LayoutParams> {
                    height = (systemBars.top + Utils.dp2Px(60, requireContext())).toInt()
                }
            binding.bigTitle
                .updateLayoutParams<ViewGroup.MarginLayoutParams> {
                    topMargin = (systemBars.top + Utils.dp2Px(60, requireContext())).toInt()
                }
            TitleAnim(
                binding.headerBar,
                binding.avatarImage,
        ...
        lifecycleScope.launch {
            val informationResult = MeParse.fetchMeData()
            val isClockIn = MeParse.isClockIn()
            ...
            withContext(Dispatchers.Main) {
                ...
                if (informationResult.cover.isNotEmpty()) {
                    ...
                    binding.bigTitle.setTextColor(ContextCompat.getColor(requireContext(), R.color.label_primary))
                    binding.userName.setTextColor(ContextCompat.getColor(requireContext(), R.color.label_primary))
                    binding.signatureText.setTextColor(ContextCompat.getColor(requireContext(), R.color.label_secondary))
                    binding.authText.setTextColor(ContextCompat.getColor(requireContext(), R.color.label_primary))
                    binding.postCount.setTextColor(ContextCompat.getColor(requireContext(), R.color.label_primary))
                    binding.postCountText.setTextColor(ContextCompat.getColor(requireContext(), R.color.label_secondary))
                    binding.resCount.setTextColor(ContextCompat.getColor(requireContext(), R.color.label_primary))
                    binding.resCountText.setTextColor(ContextCompat.getColor(requireContext(), R.color.label_secondary))
                }
        ...
        try {
            ...
            lifecycleScope.launch {
                ...
                withContext(Dispatchers.IO) {
                    // 获取推荐数据
                    myPostsResult = SearchParse.searchInfoParse(content, page.toString())
                ...
                withContext(Dispatchers.IO) {
                    ...
                    withContext(Dispatchers.Main) {
                            myPostsResult.items.let { newItems ->
                                if (adapter == nullAdapter) {
                                    // 创建新Adapter时设置点击监听
                                    val newAdapter = SearchAdapter(requireContext(), newItems.toMutableList()).apply {
                                        onItemClick = { selectedItem ->
                                            // 安全上下文检查
                                            context?.let {
